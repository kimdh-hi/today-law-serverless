...
let api_url = "https://37aq2im7bg.execute-api.ap-northeast-2.amazonaws.com/Prod"
                ...
                if (res['result'] == 'success') {
                    ...
                    let temp_html = `<div onclick="dpmenu()" class = "dropdown" >
                                        <div class = "dropdown-trigger" >
                                            ...
                                            <img alt="profile_image" src="${res['user']['profile_image']}">
...
function get_law_list() {
    ...
    $.ajax({
        type: "GET",
        url: `${api_url}/laws?offset=${offset}&query=&proposer=&condition=법안명`,
        success: function (res) {
...
function get_law_list_by_title(title) {
    ...
    $.ajax({
        type: "GET",
        url: `${api_url}/laws?offset=${offset}&query=${title}&proposer=&condition=법안명`,
        success: function (res) {
            total_count = res[0].total_count
...
function get_law_list_by_proposer_name(name) {
    ...
    $.ajax({
        type: "GET",
        url: `${api_url}/laws?offset=${offset}&proposer=${name}&query=&condition=제안자`,
        success: function (res) {
            total_count = res[0].total_count
            if (!more) g_readmore_button_show = total_count > 10 ? true : false
            add_law_list(res)
